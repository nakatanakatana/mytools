...
| `OP_AUTH_CACHE_TTL` | Duration to reuse a successful background `op whoami` lock-state check. Normal `op item` operations do not run `whoami` as a preflight. Set to `0s` to disable reuse while keeping single-flight coalescing. | `30s` |
