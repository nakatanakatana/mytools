...
// on the configured interval. Transient refresh failures, store/status
// inspection errors, and refresh-failure persistence errors use bounded
// exponential retries. Permanent authorization failures wait for the next
// periodic check without exiting the worker.
func (m Maintenance) Run(ctx context.Context) error {
	...
	for {
		...
		retryable := false
		retryClass := RefreshErrorProtocol
		if statusErr != nil {
			retryable = true
			if refreshErr != nil {
				class, _ := maintenanceFailure(refreshErr)
				retryClass = class
			...
			if refreshErr != nil {
				class, canRetry := maintenanceFailure(refreshErr)
				retryClass = class
				observer.RefreshFailed(now(), RefreshReasonMaintenance, class)
				retryable = canRetry
			} else if result.Refreshed {
		...
		if retryable {
			...
			retryAttempt++
			observer.RetryScheduled(now(), RefreshReasonMaintenance, retryClass, nextDelay)
		} else {
			retryAttempt = 0
		}
...
func maintenanceFailure(err error) (RefreshErrorClass, bool) {
	...
	return class, !refreshErr.ReauthRequired
}
