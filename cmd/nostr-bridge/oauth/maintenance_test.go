...
func (c *maintenanceClientFake) AuthorizationStatus(_ context.Context, accountDID string, period time.Duration) (Status, error) {
	...
	var err error
	switch {
	case index < len(c.statusErrs):
		err = c.statusErrs[index]
	case c.statusErr != nil:
		err = c.statusErr
	}
	return c.status, err
}
...
func TestMaintenanceRetriesUnknownRefreshFailureWithoutLeakingSecrets(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	observer := &maintenanceObserverRecorder{cancel: cancel, cancelRetries: 1}
	var timerDelays []time.Duration
	maintenance := Maintenance{
		Client:        client,
		AccountDID:    "did:plc:owner",
	...
	if len(observer.retryDelays) != 1 || observer.retryDelays[0] >= maintenance.CheckInterval {
		t.Fatalf("retry delays = %v, want one bounded delay below %s", observer.retryDelays, maintenance.CheckInterval)
	}
	if len(observer.retryClasses) != 1 || observer.retryClasses[0] != RefreshErrorProtocol {
		t.Fatalf("retry classes = %v, want protocol", observer.retryClasses)
	}
	...
	for _, class := range observer.failedClasses {
		if strings.Contains(string(class), secret) {
			t.Fatalf("failure class leaked raw secret: %q", class)
		}
	}
}

func TestMaintenanceRetriesWhenFailurePersistenceFails(t *testing.T) {
	...
	for _, class := range append(observer.failedClasses, observer.retryClasses...) {
		if strings.Contains(string(class), secret) {
			t.Fatalf("event class leaked raw secret: %q", class)
		}
	}
}

func TestMaintenanceRetriesAuthorizationStatusErrors(t *testing.T) {
	client := &maintenanceClientFake{
		results:    []RefreshResult{{Reason: RefreshReasonMaintenance, Refreshed: true}},
		status:     Status{AuthorizationAvailable: true},
		statusErrs: []error{errors.New("database unavailable")},
	}
	ctx, cancel := context.WithCancel(context.Background())
	observer := &maintenanceObserverRecorder{cancel: cancel, cancelRetries: 1}
	var timerDelays []time.Duration
	...
	if len(observer.checked) != 0 {
		t.Fatalf("checked events = %d, want none when status inspection fails", len(observer.checked))
	}
	if len(observer.succeeded) != 1 || observer.succeeded[0] != RefreshReasonMaintenance {
		t.Fatalf("success events = %v, want maintenance after persisted refresh", observer.succeeded)
	}
	if len(observer.retryDelays) != 1 || observer.retryClasses[0] != RefreshErrorProtocol {
		t.Fatalf("retry = delays %v classes %v, want protocol retry", observer.retryDelays, observer.retryClasses)
	}
	if len(observer.failedReasons) != 0 {
		t.Fatalf("failure events = %v, want none when only status inspection fails", observer.failedReasons)
	}
}
