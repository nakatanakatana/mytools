...
func TestHandleCallbackSucceedsWhenStatusPublishFailsAfterTokenSave(t *testing.T) {
	fixedNow := time.Unix(2_000_000_000, 0)
	observer := &clientObserverRecorder{}
	var state string
	var server *httptest.Server
	...
	client.store = &failReadAfterSaveOAuthStore{OAuthStore: stateStore, err: errors.New("database unavailable")}
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/oauth/callback?state="+url.QueryEscape(state)+"&code=authorization-code&iss="+url.QueryEscape(server.URL), nil)
	client.HandleCallback(recorder, request)
	if recorder.Code != http.StatusSeeOther {
	...
	if token.UpdatedAt != fixedNow.Unix() || token.LastRefreshAt != fixedNow.Unix() {
		t.Fatalf("persisted token timestamps = updated %d refresh %d", token.UpdatedAt, token.LastRefreshAt)
	}
	if len(observer.successes) != 1 || observer.successes[0] != RefreshReasonAuthorizationCode {
		t.Fatalf("callback refresh successes = %#v, want authorization_code", observer.successes)
	}
	...
	if len(observer.statuses) != 0 {
		t.Fatalf("callback status updates = %d, want none when status publish fails", len(observer.statuses))
	}
}
...
type failReadAfterSaveOAuthStore struct {
	bridgestore.OAuthStore
	err       error
	saved     bool
	saveCalls int
...
func (s *failReadAfterSaveOAuthStore) SaveOAuthToken(ctx context.Context, scope bridgestore.SourceScope, token bridgestore.OAuthToken) error {
	s.saveCalls++
	if err := s.OAuthStore.SaveOAuthToken(ctx, scope, token); err != nil {
		return err
	}
	s.saved = true
	return nil
...
func (s *failReadAfterSaveOAuthStore) OAuthTokenByAccountDID(ctx context.Context, scope bridgestore.SourceScope, accountDID string) (bridgestore.OAuthToken, error) {
	if s.saved {
		return bridgestore.OAuthToken{}, s.err
	}
	return s.OAuthStore.OAuthTokenByAccountDID(ctx, scope, accountDID)
}
