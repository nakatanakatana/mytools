...
func TestHealthReadinessUsesBlueskyOAuthAuthorizationAvailability(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			set: func(m *ProviderHealthMetrics) {
				m.AuthorizationAvailable = false
		...
		{
			name: "reauth required rejects inconsistent authorization_available",
			set: func(m *ProviderHealthMetrics) {
				m.AuthorizationAvailable = true
				m.ReauthRequired = true
			},
		...
		{
			name: "authorization unavailable is not ready without reauth flag",
			set: func(m *ProviderHealthMetrics) {
				m.AuthorizationAvailable = false
			},
			want: http.StatusServiceUnavailable,
